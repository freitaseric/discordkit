...
O sistema de módulos do Go resolverá automaticamente as dependências do
DiscordKit.
...
Substitua `example.com/my-discord-bot` pelo caminho real do módulo que pretende
utilizar.

## Instale o DiscordKit

Adicione DiscordKit ao projeto:
...
Aplicações existentes podem adotá-lo gradualmente:
...
Handlers existentes do discordgo podem coexistir com handlers gerenciados pelo
DiscordKit enquanto a aplicação é migrada aos poucos.

## Versões durante o desenvolvimento

Antes de DiscordKit atingir uma API estável `v1`, novas versões podem introduzir
mudanças incompatíveis.

Para manter builds reproduzíveis, mantenha `go.mod` e `go.sum` versionados e,
quando necessário, utilize uma versão explícita:
